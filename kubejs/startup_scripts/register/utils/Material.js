...
StartupEvents.registry("item", (event) => {
	materials.forEach((material) => {
		material.types.forEach((entry) => {
			...
			if (type === "dirty") {
				item = event.create(`cmc:dirty_${material.name}_dust`)
					.modelJson({
						parent: "minecraft:item/generated",
						textures: {
							layer0: "cmc:item/material/color/dirty/0",
							layer1: "cmc:item/material/color/dirty/1"
						}
					})
				...
				item = event.create(`cmc:${material.name}_${type}`)
					.texture(`cmc:item/material/color/${type}`)
					.color(0, material.color1)
					.tag(`mekanism:${type}s`)
...
StartupEvents.registry("block", (event) => {
	materials.forEach((material) => {
		material.types.forEach((entry) => {
			...
			let block = event.create(`cmc:${material.name}_block`)
			...
			if (entry.model == "false") {
				block.textureAll(`cmc:block/material/color/storage_blocks`)
			...
			if (material.metal) {
				block.tag(`cmc:metals`)
			}
		})
	})
	console.log("Blocks 已注册完毕!")
})
...
StartupEvents.registry("fluid", (event) => {
	materials.forEach((material) => {
		material.types.forEach((entry) => {
			...
			let molten = event.create(`cmc:molten_${material.name}`)

			molten.thinTexture(material.color1)
			molten.bucketColor(material.color1)
			molten.flowingTexture(`cmc:fluid/base/flow`)
			molten.stillTexture(`cmc:fluid/base/still`)
			...
			molten.bucketItem.modelJson({
				...
				"fluid": `cmc:molten_${material.name}`
			})

			// if (Platform.isClientEnvironment()) {
			// 	let file = `kubejs/assets/cmc/models/item/molten_${material.name}_bucket.json`
			// 	JsonIO.write(file, {
			// 		parent: "forge:item/bucket_drip",
			// 		loader: "forge:fluid_container",
			// 		fluid: `cmc:molten_${material.name}`
